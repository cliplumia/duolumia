...
export async function POST({ request, platform, cookies }) {
  try {
    ...
    if (existing) {
      ...
      if (isAdmin) {
        await BD.prepare('INSERT INTO utilisateurs (id, google_id, email, nom, plan, images_restantes, videos_restantes, voices_restantes, chat_restantes, essai_expire_a, cree_a, mis_a_jour_a) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')
          .bind(id, payload.sub, payload.email, payload.name || '', 'admin', 999999, 999999, 999999, 999999, null, now, now).run();
      } else {
        ...
        await BD.prepare('INSERT INTO utilisateurs (id, google_id, email, nom, plan, images_restantes, videos_restantes, voices_restantes, chat_restantes, essai_expire_a, cree_a, mis_a_jour_a) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')
          .bind(id, payload.sub, payload.email, payload.name || '', planInitial, imagesInitiales, videosInitiales, voixInitiales, chatsInitiaux, essaiExpire, now, now).run();
      }
    }
